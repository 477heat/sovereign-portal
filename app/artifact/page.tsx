...
export default function ArtifactPage() {
  ...
  return (
    <main className="info-control-page relative min-h-screen overflow-hidden bg-black text-white">
      <TunnelBackdrop layer="page" variant="diffused" />

      <div className="relative z-10 mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-5 md:px-8">
        <nav className="control-surface flex flex-wrap items-center justify-between gap-3 border-b border-cyan-200/15 bg-black/80 px-4 py-4 text-xs font-semibold uppercase tracking-[0.2em] text-white/70 shadow-[0_18px_50px_rgba(0,0,0,0.45)] backdrop-blur-xl md:text-sm">
        ...
        <section className="grid flex-1 items-center gap-6 py-8 2xl:grid-cols-[250px_minmax(0,1fr)_250px]">
          <aside className="control-surface hidden border border-cyan-200/25 bg-black/45 p-4 2xl:block">
            <div className="space-y-4">
              {leftGlyphs.map((glyph) => (
                <div
                  key={glyph}
                  className="control-surface-soft flex h-16 items-center justify-center border border-cyan-200/25 text-[10px] uppercase tracking-[0.24em] text-cyan-100/70"
                >
                  {glyph}
                </div>
              ))}
          ...
          <section className="control-surface control-surface-large relative overflow-hidden border border-cyan-200/30 bg-black/60 shadow-[0_0_90px_rgba(80,190,255,0.18)]">
            ...
            <div className="relative z-10 grid min-h-[680px] gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_330px] md:p-8">
                ...
                <div className="mt-8 grid gap-4 md:grid-cols-2">
                  <label className="block">
                    ...
                    <input
                      ...
                      className="control-input-surface w-full border border-cyan-200/25 bg-black/75 px-4 py-3 text-sm text-white outline-none focus:border-cyan-100"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-2 block text-[10px] uppercase tracking-[0.26em] text-cyan-100/65">
              ...
              <aside className="control-surface control-surface-strong border border-yellow-200/35 bg-black/72 p-5">
                <div className="text-[10px] uppercase tracking-[0.3em] text-yellow-100/70">
                  Preview Card
                </div>
                <div className="control-surface-soft mt-5 border border-yellow-200/25 p-4">
                ...
                <div className="mt-5">
                  ...
                  <div className="control-surface-soft mt-2 border border-fuchsia-200/35 bg-fuchsia-300/10 p-3 text-sm uppercase tracking-[0.14em] text-fuchsia-100">
          ...
          <aside className="control-surface hidden border border-fuchsia-200/25 bg-black/45 p-4 2xl:block">
            <div className="space-y-4">
              {rightGlyphs.map((glyph) => (
                <div
                  key={glyph}
                  className="control-surface-soft flex h-16 items-center justify-center border border-fuchsia-200/25 text-[10px] uppercase tracking-[0.24em] text-fuchsia-100/70"
                >
                  {glyph}
                </div>
              ))}
